...
const ConferenceStatus = ({ registration, onEdit }) => {
	...
	const getStatusConfig = (status) => {
		const configs = {
			...
			'approved': {
				...
				description: language === 'ar' 
					? 'مبروك! تم قبولك للمشاركة في المؤتمر' 
	...
	const getSessionText = (session) => {
		const sessions = {
			'first': language === 'ar' ? 'الجلسة الأولى' : 'First Session',
			'second': language === 'ar' ? 'الجلسة الثانية' : 'Second Session',
			'both': language === 'ar' ? 'كلا الجلستين' : 'Both Sessions'
		};
			...
			<div className="bg-white rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-500 animate-in slide-in-from-top-4">
				...
				<div className="p-5 sm:p-8 lg:p-12">
					<div className="flex items-center mb-6 sm:mb-8">
						<div className="bg-gradient-to-r from-green-500 to-teal-500 text-white p-3 sm:p-4 rounded-2xl mr-3 sm:mr-4">
							<span className="text-xl sm:text-2xl">🎤</span>
						</div>
						<h3 className="text-2xl sm:text-3xl font-bold text-gray-900">
							{language === 'ar' ? 'تفاصيل التسجيل في المؤتمر' : 'Conference Registration Details'}
						</h3>
					</div>
					...
					<div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
								...
								{registration.organization && (
									<div className="bg-gradient-to-r from-orange-50 to-yellow-50 p-3 sm:p-4 rounded-2xl border border-orange-100 hover:shadow-md transition-shadow duration-200">
										<div className="flex justify-between items-center gap-3">
											...
											<span className="text-gray-900 font-medium break-words text-sm sm:text-base">{registration.organization}</span>
										</div>
									</div>
								)}
							</div>
						...
						<div className="space-y-4 sm:space-y-6">
							<div className="flex items-center space-x-3 rtl:space-x-reverse">
								...
								<h4 className="text-lg sm:text-xl font-bold text-gray-800">
									{language === 'ar' ? 'معلومات المؤتمر' : 'Conference Information'}
								</h4>
							</div>
							...
							<div className="space-y-3 sm:space-y-4">
								...
								<div className="bg-gradient-to-r from-pink-50 to-rose-50 p-3 sm:p-4 rounded-2xl border border-pink-100 hover:shadow-md transition-shadow duration-200">
									<span className="font-semibold text-gray-700 flex items-center mb-2 sm:mb-3 text-sm sm:text-base">
										<span className="mr-2">📅</span>
										{language === 'ar' ? 'تاريخ المؤتمر:' : 'Conference Date:'}
									</span>
									<span className="text-gray-900 font-medium bg-white/50 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg inline-block break-words text-sm sm:text-base">
								...
								<div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-3 sm:p-4 rounded-2xl border border-emerald-100 hover:shadow-md transition-shadow duration-200">
									...
									<span className="text-gray-900 font-medium bg-white/50 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-lg inline-block break-words text-sm sm:text-base">
										{language === 'ar' ? 'الدقم - عمان' : 'Duqm - Oman'}
									</span>
								</div>
							</div>
						</div>
			...
			{registration.status === 'approved' && (
				<div className="mt-6 sm:mt-8 space-y-6 sm:space-y-8">
						...
						<div className="flex items-start space-x-3 sm:space-x-4 rtl:space-x-reverse">
							...
							<div className="flex-1">
								...
								<p className="text-green-700 text-base sm:text-lg leading-relaxed">
									{language === 'ar' 
										? 'تم قبولك للمشاركة في المؤتمر. ستتلقى مزيداً من التفاصيل حول المؤتمر قريباً.'
										: 'You have been accepted to participate in the conference. You will receive more details about the conference soon.'
									}
								</p>
							</div>
						</div>
						...
						<div className="p-5 sm:p-8 lg:p-12">
							<div className="text-center mb-6 sm:mb-8">
								<div className="flex justify-center mb-3 sm:mb-4">
									<div className="bg-gradient-to-r from-green-500 to-teal-500 text-white p-3 sm:p-4 rounded-2xl shadow-lg">
								...
								<p className="text-gray-600 text-base sm:text-lg max-w-2xl mx-auto">
									{language === 'ar' 
										? 'احتفظ بهذا QR Code لتسهيل عملية التسجيل والدخول للمؤتمر'
										: 'Keep this QR Code for easy registration and conference access'
									}
								</p>
							</div>
					...
					<div className="mt-6 sm:mt-8 text-center hidden sm:block">
								<div className="bg-gradient-to-r from-green-50 to-emerald-50 p-5 sm:p-6 rounded-2xl border border-green-200">
									<h4 className="text-base sm:text-lg font-bold text-green-800 mb-2 sm:mb-3">
										{language === 'ar' ? 'كيفية الاستخدام:' : 'How to Use:'}
									</h4>
									<div className="space-y-1.5 sm:space-y-2 text-green-700">
										...
										<p className="flex items-center justify-center space-x-2 rtl:space-x-reverse text-sm sm:text-base">
											<span>2️⃣</span>
											<span>{language === 'ar' ? 'أظهرها عند وصولك للمؤتمر' : 'Show it when you arrive at the conference'}</span>
										</p>
										<p className="flex items-center justify-center space-x-2 rtl:space-x-reverse text-sm sm:text-base">
											<span>3️⃣</span>
